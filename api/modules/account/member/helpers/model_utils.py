...
class MemberModelUtils:
    ...
    def get_list_member_from_remote_db(self) -> list:
    ...
    def get_member_from_remote_db(self, member_remote_id: str) -> dict:
        with pymssql.connect(server=server, user=user, password=password, database=database) as conn:
            with conn.cursor(as_dict=True) as cursor:
                cursor.execute("""
                    SELECT  MemberID,
    ...
    def update_member_from_remote_db(self, member_remote_id: str, data) -> dict:

        data_tpl = data.items()
        lst_key = ""
        lst_value = ()
        for item in data_tpl:
            lst_key += str(item[0]) + "= %s,"
            lst_value += (item[1],)
        lst_key = lst_key.rstrip(",")
        lst_value += (member_remote_id,)
        
        raw_execute = f"UPDATE dbo.MemberList SET " + lst_key + " WHERE MemberID =%s"
        try:
            with pymssql.connect(server=server, user=user, password=password, database=database) as conn:
                with conn.cursor(as_dict=True) as cursor:
                    cursor.execute(raw_execute, lst_value)
                    conn.commit()
                    conn.close()
        except:
            raise Exception("Incorect field name.") 
        member = self.get_member_from_remote_db(member_remote_id)

        return member

    def remove_member_from_remote_db(self, member_remote_id: str, data) -> dict:
        with pymssql.connect(server=server, user=user, password=password, database=database) as conn:
            with conn.cursor(as_dict=True) as cursor:
                cursor.execute("""
                    DELETE FROM dbo.MemberList
                    WHERE MemberID =%s
                """, member_remote_id)
                member = cursor.fetchone()
                conn.close()
    ...
    def convert_key_member(self, member, member_remote):
        member["full_name"] = member_remote["MemberName"] 
        member["address"] = member_remote["AddressVN"]
        member["email"] = member_remote["EmailVN"]
        member["phone_number"] = member_remote["TelVN"]
        member["dob"] = member_remote["Dateofbirth"]
        member["gender"] = member_remote["Gender"]
        member["register_date"] = member_remote["JoinDate"]
        member["expire_date"] = member_remote["CancelDate"]
        member["avatar"] = member_remote["Picture"]
