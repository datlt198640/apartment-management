...
class SubserviceCategoryViewSet(GenericViewSet):
    ...
    def list(self, request):
        ...
        serializer = SubserviceCategorySr(queryset, many=True)
    ...
    def retrieve(self, request, pk=None):
        obj = get_object_or_404(SubserviceCategory, pk=pk)
        serializer = SubserviceCategorySr(obj)
        return ResUtils.res(serializer.data)

    @transaction.atomic
    @action(methods=["post"], detail=True)
    def add(self, request):
        serializer = SubserviceCategorySr(data=request.data)
    ...
    def change(self, request, pk=None):
        obj = get_object_or_404(SubserviceCategory, pk=pk)
        data = request.data
        srs = SubserviceCategorySr(obj, data=data, partial=True)
