...
from ..helpers.srs import DishSr
...
class DishViewSet(GenericViewSet):
    ...
    def list(self, request):
        ...
        serializer = DishSr(queryset, many=True)
    ...
    def retrieve(self, request, pk=None):
        ...
        serializer = DishSr(obj)
        return ResUtils.res(serializer.data)

    @transaction.atomic
    @action(methods=["post"], detail=True)
    def add(self, request):
        serializer = DishSr(data=request.data)
    ...
    def change(self, request, pk=None):
        obj = get_object_or_404(Dish, pk=pk)
        data = request.data
        srs = DishSr(obj, data=data, partial=True)
