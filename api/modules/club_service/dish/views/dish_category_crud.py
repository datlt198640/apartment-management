...
from ..helpers.srs import DishCategorySr


class DishCategoryViewSet(GenericViewSet):

    _name = "dishcategory"
    permission_classes = (CustomPermission,)
    serializer_class = DishCategorySr
    search_fields = ["title"]
    ...
    def list(self, request):
        ...
        serializer = DishCategorySr(queryset, many=True)
    ...
    def retrieve(self, request, pk=None):
        obj = get_object_or_404(DishCategory, pk=pk)
        serializer = DishCategorySr(obj)
        return ResUtils.res(serializer.data)

    @transaction.atomic
    @action(methods=["post"], detail=True)
    def add(self, request):
        serializer = DishCategorySr(data=request.data)
    ...
    def change(self, request, pk=None):
        obj = get_object_or_404(DishCategory, pk=pk)
        data = request.data
        srs = DishCategorySr(obj, data=data, partial=True)
    ...
    def delete(self, request, pk=None):
        item = get_object_or_404(DishCategory, pk=pk)
        item.delete()
